...
angular.module('esn')
    .controller('profileCtrl', function ($scope, $http, $routeParams, $Auth, fileUpload) {
        $scope.setRightMenuSrc('views/profile/profilemenu.html');
        $scope.updateView('Profile_Posts', false);
    ...
    controller('profileMenuCtrl', function ($scope) {
        ...
        $scope.selectMenu = function (index) {
            if (index >= 0 && index < $scope.data.menus.length) {
                ...
                $scope.updateView('Profile_' + $selectedMenu, true);
            }
        };
            ...
            $http.post('http://imadevelopers.com/esn/posts/edit/' + $post.id + '.json', {'Post': $post}, {withCredentials: true})
                .success(function (data) {
                    if (data.result.success) {
                        ...
                        console.log(data);
                        resetFileStatus();
                        $scope.savingPost[$post.id] = false;
